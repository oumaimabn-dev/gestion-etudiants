...
const SurveillanceDirector = () => {
  ...
  const fetchRecords = async () => {
    try {
      const res = await fetch(
        "http://localhost:5000/api/surveillances/director",
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleSave = async (e) => {
    ...
    try {
      const url = editingId
        ...
        : "http://localhost:5000/api/surveillances";
  ...
  const handleEdit = (r) => {
    ...
    setPrenom(r.enseignant?.fullname.split(" ")[0] || "");
    setNom(r.enseignant?.fullname.split(" ").slice(1).join(" ") || "");
...
export default SurveillanceDirector;
